...
$(function(){
	...
	$("#sel1 option").each(function(){
		if($(this).val()=="${community.business_type}"){
	...
	$("#sel2 option").each(function(){
		if($(this).val()=="${community.location}"){
			$(this).attr("selected","selected");
		};
...
<body>
	<div class="boardhead">
		...
		<div class="warp_boardhead">
			<h3>${community.business_type } 수정</h3>
		</div>
	</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-11">
					<input type="text" class="form-control" id="subject" name="subject"
						placeholder="제목을 입력하세요" value="${community.subject }">
				</div>
			</div>
...
${pageContent.request.contextPath}
	...
	<script>
		$(function() {
			...
			CKEDITOR.instances.ckeditor.setData('<p>${community.content}</p>');//자동엔터키방지
