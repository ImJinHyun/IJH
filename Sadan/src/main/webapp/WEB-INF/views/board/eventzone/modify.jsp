...
$(function(){
	...
	$("#sel1 option").each(function(){
		if($(this).val()=="${eventzone.business_type}"){
	...
	$("#sel2 option").each(function(){
		if($(this).val()=="${eventzone.location}"){
			$(this).attr("selected","selected");
		};
...
<body>
	<div class="boardhead">
		...
		<div class="warp_boardhead">
			<h3>${eventzone.business_type } 수정</h3>
		</div>
	</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-11">
					<input type="text" class="form-control" id="subject" name="subject"
						placeholder="제목을 입력하세요" value="${eventzone.subject }">
				</div>
			</div>
...
${pageContent.request.contextPath}
	...
	<script>
		$(function() {
			...
			CKEDITOR.instances.ckeditor.setData('<p>${eventzone.content}</p>');//자동엔터키방지
