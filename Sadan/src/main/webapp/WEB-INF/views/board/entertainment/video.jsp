...
<%-- 				<td style="text-align: left;"><a href="board_read.do?no=${entertainment.no }"> ${entertainment.subject}</a></td> --%>
		...
		<hr/>
		<a class="btn btn-default pull-right" href="torentoinsert.do?type=움짤/동영상">글쓰기</a>		
		<div class="text-center">
			<ul class="pagination">
			<!-- 이전 페이지 처리 -->
						<c:if test="${pageMaker.prev }">
							<li><a href="video.do?page=${pageMaker.startPage -1 }&business_type=움짤/동영상">&laquo;</a></li>
						</c:if>
						...
						<c:forEach begin="${pageMaker.startPage }"
							...
							<li ${pageMaker.cri.page==idx?"class='active'":"" }>
								<a href="video.do${pageMaker.makeQuery(idx) }&business_type=움짤/동영상">${idx }</a>
							</li>
						</c:forEach>
						
				<!-- 다음 페이지 처리 -->
						<c:if test="${pageMaker.next }">
							<li><a href="video.do?page=${pageMaker.endPage + 1 }&business_type=움짤/동영상">&raquo;</a></li>
						</c:if>
			</ul>
		</div>	
</body>
